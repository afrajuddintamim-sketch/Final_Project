...
typedef struct {  // For both sign up and log in interfaces
...
typedef struct { // For product information
    char type[10];
    char name[50];
    char model[50];        
    int price;
...
product_information items[MAX_ITEMS]; // Global array to store products
int count = 0;

void sign_Up_interface(Sign_Up_Input *sign_up_input); // Handles the sign-up and log-in menu and logic
void sign_Up(Sign_Up_Input *sign_up_input);
void Log_In(Sign_Up_Input *sign_up_input);

void gadget_interface(void); // Main menu for gadget management (search, show, add, delete)

void makeUpperCase(char *word); // Utility function to convert a string to uppercase
void clearLeftoverInput(void); // Utility function to clear the input buffer after reading input to prevent issues with subsequent inputs
void printDashes(int howMany); // Utility function to print a line of dashes for formatting output

void featureLabels(const char *type,char *label1,char *label2,int  *featureCount); // Utility function to set the feature labels and count based on the product type (TV, PHONE, LAPTOP)

void loadFromFile(void); // Loads product information from "Gadget_info.txt" into the global items array at the start of the gadget interface
void saveToFile(void); // Saves the current product information from the global items array back to "Gadget_info.txt" whenever changes are made (adding or deleting products)

void addProduct(void); // Handles the logic for adding a new product, including input validation and saving to file
void deleteProduct(void); // Handles the logic for deleting a product by name, including searching for the product and updating the file
void showAllByType(void); // Displays all products of a specified type (TV, PHONE, LAPTOP) in a formatted table, including their features and price

int compareByPrice(const void *a, const void *b); // Comparison function for qsort to sort products by price in ascending order

void searchProducts(void); // Handles the logic for searching products based on user-defined criteria (type, budget, desired features, tolerance) and displays matching products sorted by price
...
void loadFromFile(void)
{
    ...
    while (count < MAX_ITEMS &&
           fscanf(file,"%9s %49s %49s %d %d %d",
                  items[count].type,
                  items[count].name,
                  items[count].model,
                  &items[count].price,
                  &items[count].feature,
                  &items[count].feature2) == 6)
    {
...
void showAllByType(void)
{
    ...
    if (featureCount == 2)
        printf("%-20s %-20s %-12s %-10s %-10s\n", "Name", "Model", "Price", label1, label2);
    else
        printf("%-20s %-20s %-12s %-10s\n", "Name", "Model", "Price", label1);
    ...
    for (int i = 0; i < count; i++) 
    {
        ...
        {
            if (featureCount == 2)
                printf("%-20s %-20s %-12d %-10d %-10d\n",
                       items[i].name, items[i].model, items[i].price,
                       items[i].feature, items[i].feature2);
            else
                printf("%-20s %-20s %-12d %-10d\n",
                       items[i].name, items[i].model, items[i].price, items[i].feature);
            found++;
        }
    }
